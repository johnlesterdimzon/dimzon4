...
def edit():
    edit = Tk()
    edit.title('Update Record from Database')
    edit.geometry("500x500")

    conn = sqlite3.connect('C:/Users/STUDENTS/dimzon.db')
    c = conn.cursor()
    ...
    c.execute("SELECT * FROM janjan WHERE oid=?", (record_id,))
    record = c.fetchone()
...
def submit():
    ...
    c.execute("INSERT INTO janjan VALUES (:f_name, :l_name, :age, :address, :email)",
              {
                'f_name': f_name.get(),
                'l_name': l_name.get(),
                'age': age.get(),
...
def query():
    conn = sqlite3.connect('C:/Users/STUDENTS/dimzon.db')
    c = conn.cursor()
    c.execute("SELECT *, oid FROM janjan")
    records = c.fetchall()
    conn.close()
...
def delete():
    conn = sqlite3.connect('C:/Users/STUDENTS/dimzon.db')
    c = conn.cursor()
    c.execute("DELETE FROM janjan WHERE oid=?", (delete_box.get(),))
    conn.commit()
